...
from instock.core.strategy import chan_lun

RATE_FIELDS_COUNT = 100  # N日收益率字段数目，即N值


TABLE_CN_STOCK_ATTENTION = {
    'name': 'cn_stock_attention',
    'cn': '我的关注',
    'columns': {
        'datetime': {'type': DATETIME, 'cn': '时间', 'size': 0},
...
TABLE_CN_STOCK_SPOT_BUY = {
    'name': 'cn_stock_spot_buy',
    'cn': '基本面选股',
...
TABLE_CN_STOCK_BLOCKTRADE = {
    'name': 'cn_stock_blocktrade',
    'cn': '股票大宗交易',
    'columns': {
        'date': {'type': DATE, 'cn': '日期', 'size': 0},
        'code': {'type': NVARCHAR(6), 'cn': '代码', 'size': 60},
...
TABLE_CN_STOCK_FOREIGN_KEY = {
    'name': 'cn_stock_foreign_key',
    'cn': '股票外键',
    'columns': {
        'date': {'type': DATE, 'cn': '日期', 'size': 0},
        'code': {'type': NVARCHAR(6), 'cn': '代码', 'size': 60},
...
TABLE_CN_STOCK_BACKTEST_DATA = {
    'name': 'cn_stock_backtest_data',
    'cn': '股票回归测试数据',
    'columns': {
        'rate_%s' % i: {'type': FLOAT, 'cn': '%s日收益率' % i, 'size': 100} for i in range(1, RATE_FIELDS_COUNT + 1, 1)}
}
...
TABLE_CN_STOCK_STRATEGIES = [
    {
        ...
        'cn': '放量上涨',
        'size': 70,
        'func': enter.check_volume,
        'columns': _tmp_columns
    },
    {
        'name': 'cn_stock_strategy_keep_increasing',
        'cn': '均线多头',
        'size': 70,
        'func': keep_increasing.check,
        'columns': _tmp_columns
    },
    {
        'name': 'cn_stock_strategy_parking_apron',
        'cn': '停机坪',
        'size': 70,
        'func': parking_apron.check,
        'columns': _tmp_columns
    },
    {
        'name': 'cn_stock_strategy_backtrace_ma250',
        'cn': '回踩年线',
        'size': 70,
        'func': backtrace_ma250.check,
        'columns': _tmp_columns
    },
    {
        'name': 'cn_stock_strategy_breakthrough_platform',
        'cn': '突破平台',
        'size': 70,
    ...
    {
        'name': 'cn_stock_strategy_low_backtrace_increase',
        'cn': '无大幅回撤',
        'size': 70,
    ...
    {
        'name': 'cn_stock_strategy_turtle_trade',
        'cn': '海龟交易法则',
        'size': 70,
        'func': turtle_trade.check_enter,
        'columns': _tmp_columns
    },
    {
        'name': 'cn_stock_strategy_high_tight_flag',
        'cn': '高而窄的旗形',
        'size': 70,
    ...
    {
        'name': 'cn_stock_strategy_climax_limitdown',
        'cn': '放量跌停',
        'size': 70,
        'func': climax_limitdown.check,
        'columns': _tmp_columns
    },
    {
        'name': 'cn_stock_strategy_low_atr',
        'cn': '低ATR成长',
        'size': 70,
        'func': low_atr.check_low_increase,
        'columns': _tmp_columns
    },
    {
        'name': 'cn_stock_strategy_chan_lun',
        'cn': '缠论选股',
        'size': 70,
        'func': chan_lun.check,
        'columns': _tmp_columns
...
STOCK_KLINE_PATTERN_DATA = {
    'name': 'cn_stock_pattern_recognitions',
    'cn': 'K线形态',
    'columns': {
        'tow_crows': {'type': SmallInteger, 'cn': '两只乌鸦', 'size': 70, 'func': tl.CDL2CROWS},
        'upside_gap_two_crows': {'type': SmallInteger, 'cn': '向上跳空的两只乌鸦', 'size': 70, 'func': tl.CDLUPSIDEGAP2CROWS},
        'three_black_crows': {'type': SmallInteger, 'cn': '三只乌鸦', 'size': 70, 'func': tl.CDL3BLACKCROWS},
        'identical_three_crows': {'type': SmallInteger, 'cn': '三胞胎乌鸦', 'size': 70, 'func': tl.CDLIDENTICAL3CROWS},
        'three_line_strike': {'type': SmallInteger, 'cn': '三线打击', 'size': 70, 'func': tl.CDL3LINESTRIKE},
        'dark_cloud_cover': {'type': SmallInteger, 'cn': '乌云压顶', 'size': 70, 'func': tl.CDLDARKCLOUDCOVER},
        'evening_doji_star': {'type': SmallInteger, 'cn': '十字暮星', 'size': 70, 'func': tl.CDLEVENINGDOJISTAR},
        ...
        'modified_hikkake_pattern': {'type': SmallInteger, 'cn': '修正陷阱', 'size': 70, 'func': tl.CDLHIKKAKEMOD},
        ...
        'shooting_star': {'type': SmallInteger, 'cn': '射击之星', 'size': 70, 'func': tl.CDLSHOOTINGSTAR},
        'stalled_pattern': {'type': SmallInteger, 'cn': '停顿形态', 'size': 70, 'func': tl.CDLSTALLEDPATTERN},
        'advance_block': {'type': SmallInteger, 'cn': '大敌当前', 'size': 70, 'func': tl.CDLADVANCEBLOCK},
        'high_wave_candle': {'type': SmallInteger, 'cn': '风高浪大线', 'size': 70, 'func': tl.CDLHIGHWAVE},
        'engulfing_pattern': {'type': SmallInteger, 'cn': '吞噬模式', 'size': 70, 'func': tl.CDLENGULFING},
        'abandoned_baby': {'type': SmallInteger, 'cn': '弃婴', 'size': 70, 'func': tl.CDLABANDONEDBABY},
        'closing_marubozu': {'type': SmallInteger, 'cn': '收盘缺影线', 'size': 70, 'func': tl.CDLCLOSINGMARUBOZU},
        'doji': {'type': SmallInteger, 'cn': '十字', 'size': 70, 'func': tl.CDLDOJI},
        'up_down_gap': {'type': SmallInteger, 'cn': '向上/下跳空并列阳线', 'size': 70, 'func': tl.CDLGAPSIDESIDEWHITE},
        'long_legged_doji': {'type': SmallInteger, 'cn': '长脚十字', 'size': 70, 'func': tl.CDLLONGLEGGEDDOJI},
        'rickshaw_man': {'type': SmallInteger, 'cn': '黄包车夫', 'size': 70, 'func': tl.CDLRICKSHAWMAN},
        'marubozu': {'type': SmallInteger, 'cn': '光头光脚/缺影线', 'size': 70, 'func': tl.CDLMARUBOZU},
        'three_inside_up_down': {'type': SmallInteger, 'cn': '三内部上涨和下跌', 'size': 70, 'func': tl.CDL3INSIDE},
        'three_outside_up_down': {'type': SmallInteger, 'cn': '三外部上涨和下跌', 'size': 70, 'func': tl.CDL3OUTSIDE},
        'three_stars_in_the_south': {'type': SmallInteger, 'cn': '南方三星', 'size': 70, 'func': tl.CDL3STARSINSOUTH},
        'three_white_soldiers': {'type': SmallInteger, 'cn': '三个白兵', 'size': 70, 'func': tl.CDL3WHITESOLDIERS},
        ...
        'concealing_baby_swallow': {'type': SmallInteger, 'cn': '藏婴吞没', 'size': 70, 'func': tl.CDLCONCEALBABYSWALL},
        'counterattack': {'type': SmallInteger, 'cn': '反击线', 'size': 70, 'func': tl.CDLCOUNTERATTACK},
        'dragonfly_doji': {'type': SmallInteger, 'cn': '蜻蜓十字/T形十字', 'size': 70, 'func': tl.CDLDRAGONFLYDOJI},
        'evening_star': {'type': SmallInteger, 'cn': '暮星', 'size': 70, 'func': tl.CDLEVENINGSTAR},
        'gravestone_doji': {'type': SmallInteger, 'cn': '墓碑十字/倒T十字', 'size': 70, 'func': tl.CDLGRAVESTONEDOJI},
        ...
        'harami_cross_pattern': {'type': SmallInteger, 'cn': '十字孕线', 'size': 70, 'func': tl.CDLHARAMICROSS},
        'homing_pigeon': {'type': SmallInteger, 'cn': '家鸽', 'size': 70, 'func': tl.CDLHOMINGPIGEON},
        'inverted_hammer': {'type': SmallInteger, 'cn': '倒锤头', 'size': 70, 'func': tl.CDLINVERTEDHAMMER},
        'kicking': {'type': SmallInteger, 'cn': '反冲形态', 'size': 70, 'func': tl.CDLKICKING},
        'kicking_bull_bear': {'type': SmallInteger, 'cn': '由较长缺影线决定的反冲形态', 'size': 70, 'func': tl.CDLKICKINGBYLENGTH},
        ...
        'matching_low': {'type': SmallInteger, 'cn': '相同低价', 'size': 70, 'func': tl.CDLMATCHINGLOW},
        'mat_hold': {'type': SmallInteger, 'cn': '铺垫', 'size': 70, 'func': tl.CDLMATHOLD},
        'morning_doji_star': {'type': SmallInteger, 'cn': '十字晨星', 'size': 70, 'func': tl.CDLMORNINGDOJISTAR},
        'morning_star': {'type': SmallInteger, 'cn': '晨星', 'size': 70, 'func': tl.CDLMORNINGSTAR},
        'piercing_pattern': {'type': SmallInteger, 'cn': '刺透形态', 'size': 70, 'func': tl.CDLPIERCING},
        'rising_falling_three': {'type': SmallInteger, 'cn': '上升/下降三法', 'size': 70, 'func': tl.CDLRISEFALL3METHODS},
        'separating_lines': {'type': SmallInteger, 'cn': '分离线', 'size': 70, 'func': tl.CDLSEPARATINGLINES},
        'short_line_candle': {'type': SmallInteger, 'cn': '短蜡烛', 'size': 70, 'func': tl.CDLSHORTLINE},
        'spinning_top': {'type': SmallInteger, 'cn': '纺锤', 'size': 70, 'func': tl.CDLSPINNINGTOP},
        'stick_sandwich': {'type': SmallInteger, 'cn': '条形三明治', 'size': 70, 'func': tl.CDLSTICKSANDWICH},
        'takuri': {'type': SmallInteger, 'cn': '探水竿', 'size': 70, 'func': tl.CDLTAKURI},
        'tasuki_gap': {'type': SmallInteger, 'cn': '跳空并列阴阳线', 'size': 70, 'func': tl.CDLTASUKIGAP},
        'tristar_pattern': {'type': SmallInteger, 'cn': '三星', 'size': 70, 'func': tl.CDLTRISTAR},
        'unique_3_river': {'type': SmallInteger, 'cn': '奇特三河床', 'size': 70, 'func': tl.CDLUNIQUE3RIVER},
        'upside_downside_gap': {'type': SmallInteger, 'cn': '上升/下降跳空三法', 'size': 70, 'func': tl.CDLXSIDEGAP3METHODS}
    }
}

TABLE_CN_STOCK_KLINE_PATTERN = {
    'name': 'cn_stock_pattern',
    'cn': '股票K线形态',
    'columns': TABLE_CN_STOCK_FOREIGN_KEY['columns'].copy()
}
...
def get_field_type_name(col_type):
    """
    获取数据表中各个字段的类型分类，返回结果有三种，即datetime、numeric、string
    """
    if col_type == DATE:
        return "datetime"
