...
def prepare(date):
    """
    计算股票的指标数据，如macd、kdj等数据，并完成数据入库
    :param date:
    :return:
    """
    try:
        stocks_data = stock_hist_data(date=date).get_data()   # 读取指定日期的股票
...
def guess_buy(date):
    """
    股票指标买入模块：对每日指标数据进行筛选，将符合条件的二次筛选出来，并保存到数据表中
    :param date:
    :return:
    """
    try:
...
def guess_sell(date):
    """
    股票指标卖出模块：对每日指标数据进行筛选，将符合条件的二次筛选出来，并保存到数据表中
    :param date:
    :return:
    """
    try:
...
def main():
    """
    计算股票指标数据并筛选买入、卖出股票清单的主入口，整体实现Web页面中“股票指标数据“模块中的数据整理过程
    :return:
    """
    runt.run_with_args(prepare)   # 计算股票指标数据，如macd等
    runt.run_with_args(guess_buy)  # 根据买入指标筛选买入股票列表
    runt.run_with_args(guess_sell)   # 根据卖出指标筛选卖出股票列表
